...
/**
 * Utility function to check RPC connectivity with better error messages
 * @param url RPC URL to test
 * @returns Promise with connection status and details
 */
export async function checkRpcConnection(url: string): Promise<{ success: boolean; message: string; latency?: number }> {
  ...
  try {
    ...
    if (data.error) {
      return { 
        success: false, 
        message: `JSON-RPC error: ${data.error.message || JSON.stringify(data.error)}`,
        latency
      };
    }
